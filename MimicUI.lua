...
RunService.Heartbeat:Connect(function()
	...
	if tick() % 2 < 0.05 then enableFPSBoost() end
end)
local xrayObjects = {}
...
inspectFrame.Size = UDim2.new(0,280,0,220)
...
inspLbl.Size = UDim2.new(1,-8,1,-32); inspLbl.Position = UDim2.new(0,4,0,28)
...
posFrame.Size = UDim2.new(0,220,0,62)
posFrame.Position = UDim2.new(0,10,1,-72)
posFrame.BackgroundColor3 = Color3.fromRGB(14,14,22)
posFrame.BackgroundTransparency = 0.2
posFrame.BorderSizePixel = 0
posFrame.ZIndex = 10
posFrame.Parent = screenGui
Instance.new("UICorner",posFrame).CornerRadius = UDim.new(0,10)
local posStroke = Instance.new("UIStroke",posFrame)
posStroke.Color = Color3.fromRGB(80,100,220); posStroke.Thickness = 1.2

local posTitle = Instance.new("TextLabel",posFrame)
posTitle.Size = UDim2.new(1,0,0,20); posTitle.Position = UDim2.new(0,0,0,4)
posTitle.BackgroundTransparency = 1
posTitle.Text = "📍 Position"
posTitle.TextColor3 = Color3.fromRGB(150,170,255)
posTitle.Font = Enum.Font.GothamBold; posTitle.TextSize = 11; posTitle.ZIndex = 11

local posLbl = Instance.new("TextLabel",posFrame)
posLbl.Size = UDim2.new(1,-8,0,36); posLbl.Position = UDim2.new(0,4,0,22)
posLbl.BackgroundTransparency = 1
posLbl.Text = "X: 0  Y: 0  Z: 0"
posLbl.TextColor3 = Color3.fromRGB(200,220,255)
posLbl.Font = Enum.Font.Gotham; posLbl.TextSize = 12; posLbl.ZIndex = 11
posLbl.TextXAlignment = Enum.TextXAlignment.Left
posLbl.TextWrapped = true

-- อัปเดต position ทุก 0.5 วิ (ลด lag ตอนเดินเร็ว)
local posTimer = 0
RunService.Heartbeat:Connect(function(dt)
	posTimer += dt
	if posTimer >= 0.5 then
		posTimer = 0
		local root = character and character:FindFirstChild("HumanoidRootPart")
		if root then
			local p = root.Position
			posLbl.Text = string.format("X: %.1f\nY: %.1f  Z: %.1f", p.X, p.Y, p.Z)
		end
	end
end)
...
teleDown.MouseButton1Click:Connect(function()
	local root = character and character:FindFirstChild("HumanoidRootPart")
	if root then
		local targetPos = root.CFrame + Vector3.new(0,-50,0)
