...
scroll.CanvasSize = UDim2.new(0,0,0,640)  -- ความสูงเนื้อหาทั้งหมด
...
-- Y-Axis label
local yLabel = Instance.new("TextLabel", scroll)
yLabel.Size = UDim2.new(1,-24,0,16); yLabel.Position = UDim2.new(0,12,0,258)
...
local btnUp = Instance.new("TextButton", scroll)
btnUp.Size = UDim2.new(0.44,0,0,34); btnUp.Position = UDim2.new(0,12,0,277)
btnUp.BackgroundColor3 = Color3.fromRGB(40,60,100); btnUp.Text = "⬆  ขึ้น/กระโดด"
...
local btnDown = Instance.new("TextButton", scroll)
btnDown.Size = UDim2.new(0.44,0,0,34); btnDown.Position = UDim2.new(0.5,2,0,277)
...
btnUp.InputBegan:Connect(function(i)
	if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
		if noclipEnabled then
			...
			-- ไม่ได้เปิด noclip ให้กระโดดแทน
			local hum = getHumanoid()
...
createToggleButton(scroll, "Item Highlight", "✨", 320, function(state)
...
createToggleButton(scroll, "Hitbox ESP", "📦", 404, function(state)
	hitboxEnabled = state
	if state then refreshHitbox() else clearHitbox() end
end)

local divider = Instance.new("Frame", scroll)
divider.Size = UDim2.new(1,-24,0,1); divider.Position = UDim2.new(0,12,0,450)
...
hint.Size = UDim2.new(1,-24,0,60); hint.Position = UDim2.new(0,12,0,458)
