...
local function enableNoclip()
	...
	noclipConn = RunService.Stepped:Connect(function()
		...
		-- ป้องกันตกโลก: clamp velocity Y ไม่ให้ดิ่งลง
		local root = character:FindFirstChild("HumanoidRootPart")
		if root and root.AssemblyLinearVelocity.Y < -10 then
...
panel.Name = "Panel"
panel.Size = UDim2.new(0,290,0,435)
...
createToggleButton(panel, "Item Highlight", "✨", 304, function(state)
	itemEspEnabled = state
	if state then refreshItemESP() else clearItemESP() end
end)

local divider = Instance.new("Frame",panel)
divider.Size = UDim2.new(1,-24,0,1); divider.Position = UDim2.new(0,12,0,353)
...
hint.Size = UDim2.new(1,-24,0,60); hint.Position = UDim2.new(0,12,0,360)
hint.BackgroundTransparency = 1
hint.Text = "🔴 Monster ESP = กล่องแดงรอบมอนสเตอร์\n🟡 Item Highlight = กล่องเหลืองรอบไอเทม\n👻 Noclip = ลอดกำแพง | ลาก TitleBar เพื่อย้าย UI"
